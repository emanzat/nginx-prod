...
> **Runbook ITOps** — Guide d’implémentation pas à pas
> **Public cible :** Équipes ITOps / SRE / Plateforme OpenShift
> **Pré-requis lecteur :** Connaissance de base OpenShift (`oc`), Helm, HashiCorp Vault, TLS/PKI
...
1. [Étape 1 — Sidecar Nginx & Init Container (Pod Cert)](#3-étape-1--sidecar-nginx--init-container-pod-cert)
1. [Étape 2 — CronJob `issue-route-cert` (Route + Cert)](#4-étape-2--cronjob-issue-route-cert-route--cert)
1. [Étape 3 — CronJob `pod-cert-renewal` (Auto-renouvellement)](#5-étape-3--cronjob-pod-cert-renewal-auto-renouvellement)
1. [Étape 4 — Vérification, tests & rollback](#6-étape-4--vérification-tests--rollback)
1. [Annexe A — RBAC complet](#annexe-a--rbac-complet)
1. [Annexe B — Troubleshooting](#annexe-b--troubleshooting)
...
- [ ] ServiceAccount `cert-lifecycle-sa` pour les CronJobs (RBAC en [Annexe A](#annexe-a--rbac-complet))
...
|CronJob `pod-cert-renewal` en `Forbidden`                          |Permission manquante sur `deployments`                                       |Vérifier le RBAC ([Annexe A](#annexe-a--rbac-complet))                          |
